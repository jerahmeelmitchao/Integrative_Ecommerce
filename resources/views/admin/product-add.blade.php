...
<script>
    $(function() {
        // Image upload preview for the main image
        $("#myFile").on("change", function() {
            const [file] = this.files;
            if (file) {
                $("#imgpreview img").attr('src', URL.createObjectURL(file));
                $("#imgpreview").show(); // Show the preview of the image
            }
        });

        // Gallery image upload preview for multiple images
        ...
        // Slug generation based on the product name
        $("input[name='name']").on("change", function() {
            $("input[name='slug']").val(StringToSlug($(this).val()));
        });
    });

    // Convert product name to a slug format
